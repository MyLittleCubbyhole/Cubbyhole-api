...
provider.bandwidth = function(id, callback) {
	var query = "select p.downloadbandwidth as download, p.uploadbandwidth as upload from cubbyhole.plan p inner join cubbyhole.subscribe s on p.id = s.planid inner join cubbyhole.user u on u.id = s.userid where u.id = " + id + " and s.paused = 0 and NOW() between s.datestart and s.dateend;";
